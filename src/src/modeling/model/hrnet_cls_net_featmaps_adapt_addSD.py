...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # img_features = self.conv_layers2(img_features)
        # img_features = self.bn3(img_features)
        # img_features = self.relu(img_features)
        # img_features = self.SVD(img_features,images)#   256,28,28

            


        #x = self.relu(x) 
        # x = x + img_features_reshaped          
            # img_features = x
            # img_features = img_features.view(B, 16, 4, 4, 56, 56)
            # img_features = img_features.permute(0, 1, 4, 2, 5, 3)
            # img_features = img_features.contiguous().view(B, 16, 224, 224)
            # img_features = self.conv_layers2(img_features)
            # img_features = self.bn3(img_features)
            # img_features = self.relu(img_features)        
            # img_features = self.SVD(img_features,images)#torch.Size([2, 4, 8, 56, 56])
            # img_features_reshaped = rearrange(img_features, "b f c h w -> (b f) c h w", f=8)
            # img_features = self.conv_layers(img_features_reshaped)
            # img_features = self.bn4(img_features)
            # img_features = self.relu(img_features)
            # img_features = x    
            # img_features = self.SVD(img_features,images)
            # img_features_reshaped = rearrange(img_features, "b f c h w -> (b f) c h w", f=8)
            # img_features = self.conv_layers3(img_features_reshaped)
            # img_features = self.bn4(img_features)
            # img_features = self.relu(img_features)
            # x = x + img_features
            #x = x + img_features
