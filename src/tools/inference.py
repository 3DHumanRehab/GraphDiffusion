...
from src.modeling.model.modeling_ProGraph_EMA_4_S_adapt_train import ProGraph_Body_Network as _ProGraph_Network
...
def run_inference(args, image_list, ProGraph_model, smpl, renderer):
    ...
    for image_file in image_list:
        if 'pred' not in image_file:
            ...
            if frame_num%8==0:
                ...
                # forward-pass
                out = ProGraph_model(final_batch)
...
def parse_args():
    ...
    parser.add_argument("--model_name", default='ProGraph-L', type=str,
                        help='Transformer architecture: ProGraph-S, ProGraph-M, ProGraph-L')
...
def main(args):
    print("ProGraph for 3D Human Mesh Reconstruction!")
    global logger
    ...
    logger = setup_logger("ProGraph Inference", args.output_dir, 0)
    ...
    if (args.resume_checkpoint != None) and (args.resume_checkpoint != 'None') and ('state_dict' not in args.resume_checkpoint):
        ...
        ProGraph_Network = torch.load(args.resume_checkpoint)
    else:
        ...
        ProGraph_Network = _ProGraph_Network(args, backbone, mesh_sampler,smpl)
        # number of parameters
        overall_params = sum(p.numel() for p in ProGraph_Network.parameters() if p.requires_grad)
        ...
        if (args.resume_checkpoint != None) and (args.resume_checkpoint != 'None'):
            ...
            ProGraph_Network.load_state_dict(state_dict, strict=False)
            del state_dict
    ProGraph_Network.to(args.device)
    ...
    run_inference(args, image_list, ProGraph_Network, smpl, renderer)    
