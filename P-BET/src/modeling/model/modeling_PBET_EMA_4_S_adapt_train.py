...
class PBET_Body_Network(nn.Module):
    """PBET for 3D human pose and mesh reconstruction from a single RGB image"""
    def __init__(self, args, backbone, mesh_sampler,smpl):
        ...
        if 'PBET-S' in args.model_name:
            num_enc_layers = 1
            num_dec_layers = 1
        elif 'PBET-M' in args.model_name:
            num_enc_layers = 2
            num_dec_layers = 2
        elif 'PBET-L' in args.model_name:
            num_enc_layers = 3
            num_dec_layers = 3
        else:
