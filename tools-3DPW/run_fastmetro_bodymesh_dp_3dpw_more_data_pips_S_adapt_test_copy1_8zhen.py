...
def run_eval(args, val_dataloader, FastMETRO_model, smpl, renderer,mesh_sample):
    smpl.eval()
    ...
    run_validate2(args, val_dataloader, 
                              FastMETRO_model,
                              criterion_keypoints,
                               criterion_vertices,
                              epoch, 
...
def run_validate2(args, val_dataloader, Network, criterion_keypoints, criterion_vertices, epoch, smpl, mesh_sampler):
    ...
    with torch.no_grad():
        ...
        #checkpoint = torch.load("/root/fengzehui/output_smooth/checkpoint-0-0/state_dict.bin")
        ...
        for i, (img_keys, images, annotations) in enumerate(val_dataloader):
            ...
            for b in range(0,1):
                ...
                gt_save = gt_3d_joints[b:b+step:4,:,:3]
    ...
    # np.mean(aa) * 1000
            ...
            smooth_3d_joints = smooth_model(smooth_3d_joints.cuda())
            smooth_3d_joints = smooth_3d_joints.transpose(2,1) # torch.Size([96, 42, 32])
            # FRAMENUM=32
            # batch_size, num_points, dim = pred_3d_joints.size()
            # pred_3d_joints = pred_3d_joints.view(batch_size // FRAMENUM, FRAMENUM, num_points, dim)
            # pred_3d_joints[:, FRAMENUM-1, :, :] = (pred_3d_joints[:, FRAMENUM-2, :, :] + pred_3d_joints[:, FRAMENUM-1, :, :]) / 2.0
            ...
            #     # 计算中间三帧的平均值
            #     if k%4==1:
            #         pred_3d_joints[:, k, :, :] =((pred_3d_joints[:, k-1, :, :] - pred_3d_joints[:, k+2, :, :]) / 3.0 * 2.0 + pred_3d_joints[:, k+2, :, :])
            #         pred_3d_joints[:, k+1, :, :] =((pred_3d_joints[:, k-1, :, :] - pred_3d_joints[:, k+2, :, :]) / 3.0 * 1.0 + pred_3d_joints[:, k+2, :, :])

                    
            # smooth_3d_joints=pred_3d_joints.view(batch_size,num_points,dim) # torch.Size([32, 14, 3])
            ...
            keypoint_root = [2, 3]
            denoised_pos = denoised_pos - denoised_pos[:,keypoint_root, :].mean(axis=1).reshape(-1, 1, 3)
...
def main(args):
    ...
    if args.run_evaluation:
        run_eval(args, val_dataloader, _FastMETRO_Network, smpl, renderer,mesh_sampler)
    else:
