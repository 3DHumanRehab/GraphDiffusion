...
from src.modeling.hrnet.hrnet_cls_net_featmaps_adapt_heatmap import get_cls_net
...
def convert_crop_cam_to_orig_img(cam, bbox, img_width, img_height):
    ...
    return orig_cam
def make_all_heatmap(all_image,first_featmap,ema_out,ema_S_out,x):
    ...
    with torch.no_grad():
        ...
        for iteration, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            if args.run_eval_and_visualize:
                ...
                first_featmap = out['first_featmap']
                ema_out = out['ema_out']
                ...
                save_path = "/HOME/HOME/Zhongzhangnan/FastMETRO_EMA_adapt/test_byt_3dpw_flat_guitar_02/"
                ...
                for ii in range(len(all_vis)):
                    ...
                    visual_imgs = this_vis.transpose(0,1)
                    visual_imgs = visual_imgs.transpose(1,2)
                    visual_imgs = np.asarray(visual_imgs)
                    ...
                    if is_main_process():
                        ...
                        cnn_heatmap =  save_path + 'visual_' + stamp +"_"+'%05d'%ii+"_"+ 'cnn_heatmap.png'
                        mea_heatmap =  save_path + 'visual_' + stamp +"_"+'%05d'%ii+"_"+ 'mea_heatmap.png'
                        mes_heatmap =  save_path + 'visual_' + stamp +"_"+'%05d'%ii+"_"+ 'mes_heatmap.png'
                        final_heatmap =  save_path + 'visual_' + stamp +"_"+'%05d'%ii+"_"+ 'final_heatmap.png'

                        # if args.use_opendr_renderer:
                        #     visual_imgs[:,:,::-1] = visual_imgs[:,:,::-1]*255
                        #cat_image = cv2.cvtColor(cat_image.astype(np.uint8), cv2.COLOR_RGB2BGR)
                        mesh_image = cv2.cvtColor(mesh_image.astype(np.uint8), cv2.COLOR_RGB2BGR)
                        this_image_1000 = cv2.cvtColor(this_image_1000.astype(np.uint8), cv2.COLOR_RGB2BGR)
                        #cv2.imwrite(temp_fname_cat, cat_image)
                        cv2.imwrite(temp_fname_mesh, mesh_image)
                        #cv2.imwrite(temp_fname_org, this_image_1000)

                        #cv2.imwrite(cnn_heatmap, cv2.cvtColor(cnn_featmap[ii], cv2.COLOR_RGB2BGR))
                        #cv2.imwrite(mea_heatmap, cv2.cvtColor(ema_featmap[ii], cv2.COLOR_RGB2BGR))
                        #cv2.imwrite(mes_heatmap, cv2.cvtColor(ems_featmap[ii], cv2.COLOR_RGB2BGR))
