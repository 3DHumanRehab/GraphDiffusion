...
def parse_args():
    ...
    parser.add_argument("--resume_checkpoint", default=path+"models/fastmetro_checkpoint/FastMETRO-L-H64_smpl_h36m_state_dict.bin", type=str, required=False,
...
def main(args):
    ...
    #         del state_dict
    #hrnet_yaml = path+'models/hrnet/cls_hrnet_w64_sgd_lr5e-2_wd1e-4_bs32_x100.yaml'
    #hrnet_checkpoint = 'models/hrnet/hrnetv2_w64_imagenet_pretrained.pth'
    #hrnet_update_config(hrnet_config, hrnet_yaml)
    #backbone = get_cls_net(hrnet_config, pretrained=hrnet_checkpoint)
    logger.info('=> loading hrnet-v2-w64 model')
    _FastMETRO_Network = FastMETRO_Network(args, mesh_sampler)
    if args.resume_checkpoint!=None and args.resume_checkpoint!='None':
        # for fine-tuning or resume training or inference, load weights from checkpoint
        logger.info("Loading state dict from checkpoint {}".format(args.resume_checkpoint))
