...
# sys.path.remove('/home/fzh')
sys.path.append("/HOME/Zhongzhangnan/FastMETRO_MEA_PIPS")
# print(os.listdir("/HOME/Zhongzhangnan/FastMETRO_MEA_PIPS"))

from src.tools.loss_change import calc_loss_SingleMesh_2D3DVelocity_changeWeight_no_heatmap,mean_velocity_error_train_2
from src.modeling.model.modeling_fastmetro_EMA_4_S_cat import FastMETRO_Body_Network as FastMETRO_Network
...
def run_train(args, train_dataloader, val_dataloader, FastMETRO_model, smpl, mesh_sampler, renderer, smpl_intermediate_faces, smpl_intermediate_edges):    
    ...
    for _, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        loss_3d_mean_velocity = mean_velocity_error_train_2(pred_3d_joints_from_token,gt_3d_joints[:, :, :-1].clone())
        ...
        loss_2d_mean_velocity = mean_velocity_error_train_2(pred_2d_joints_from_token,gt_2d_joints[:, :, :-1].clone())
...
def parse_args():
    ...
    parser.add_argument("--train_yaml", default='/HOME/data/PointHMR/datasets/3dpw/train.yaml', type=str, required=False,
                        help="Yaml file with all data for training.")
    parser.add_argument("--val_yaml", default='/HOME/data/PointHMR/datasets/3dpw/test_has_gender.yaml', type=str, required=False,
    ...
    #########################################################
    parser.add_argument("--output_dir", default='output_3dpw_step8_ema_back_S_loss/', type=str, required=False,
                        help="The output directory to save checkpoint and test results.")
    parser.add_argument("--saving_epochs", default=1, type=int)
    parser.add_argument("--resume_checkpoint", default="/HOME/Zhongzhangnan/FastMETRO_MEA_PIPS/models/fastmetro_checkpoint/FastMETRO-L-H64_smpl_3dpw_state_dict.bin", type=str, required=False,
...
def main(args):
    ...
    # Mesh and SMPL utils
    smpl = SMPL()
    smpl = torch.nn.DataParallel(smpl)
    smpl.to(device)
    mesh_sampler = Mesh()
    mesh_sampler = torch.nn.DataParallel(mesh_sampler)
    ...
    _FastMETRO_Network = torch.nn.DataParallel(_FastMETRO_Network)
