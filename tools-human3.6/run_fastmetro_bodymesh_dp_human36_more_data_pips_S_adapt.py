...
def vertices_loss(criterion_vertices, pred_vertices, gt_vertices, has_smpl, device):
    ...
    if len(gt_vertices_with_shape) > 0:
        ...
        return torch.FloatTensor(1).fill_(0.).to(device) 
def vertices_loss_log(criterion_vertices, pred_vertices, gt_vertices, has_smpl, device):
    """
    Compute per-vertex loss if vertex annotations are available.
    """
    pred_vertices_with_shape = pred_vertices[has_smpl == 1]
    gt_vertices_with_shape = gt_vertices[has_smpl == 1]
    if len(gt_vertices_with_shape) > 0:
        pred_vertices_with_shape = F.log_softmax(pred_vertices_with_shape,dim=1)
        gt_vertices_with_shape = F.softmax(gt_vertices_with_shape,dim=1)
        return criterion_vertices(pred_vertices_with_shape, gt_vertices_with_shape)
    else:
        return torch.FloatTensor(1).fill_(0.).to(device) 
...
def run_train(args, train_dataloader, val_dataloader, FastMETRO_model, smpl, mesh_sampler, renderer, smpl_intermediate_faces, smpl_intermediate_edges):    
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer.module, args.lr_drop,gamma=0.2)
    ...
    for _, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        # loss_3d_vertices_fine_mean_velocity = mean_velocity_error_train_2(pred_3d_vertices_fine,gt_3d_vertices_fine.clone())
        loss_3d_vertices = (args.vertices_coarse_loss_weight * (vertices_loss(criterion_3d_vertices, pred_3d_vertices_coarse, gt_3d_vertices_coarse, has_smpl, args.device) +vertices_loss_log(log_3d_vertices, pred_3d_vertices_coarse, gt_3d_vertices_coarse, has_smpl, args.device) )+ \
                        args.vertices_intermediate_loss_weight * (vertices_loss(criterion_3d_vertices, pred_3d_vertices_intermediate, gt_3d_vertices_intermediate, has_smpl, args.device)+ vertices_loss_log(log_3d_vertices, pred_3d_vertices_intermediate, gt_3d_vertices_intermediate, has_smpl, args.device))+ \
                        args.vertices_fine_loss_weight * (vertices_loss(criterion_3d_vertices, pred_3d_vertices_fine, gt_3d_vertices_fine, has_smpl, args.device) + vertices_loss_log(log_3d_vertices, pred_3d_vertices_fine, gt_3d_vertices_fine, has_smpl, args.device) ))
        # compute edge length loss (GT supervision & self regularization) & normal vector loss
        loss_edge_normal = (args.edge_gt_loss_weight * edge_gt_loss(pred_3d_vertices_intermediate, gt_3d_vertices_intermediate, has_smpl, args.device) + \
                        args.edge_self_loss_weight * edge_self_loss(pred_3d_vertices_intermediate, has_smpl, args.device) + \
                        args.normal_loss_weight * normal_loss(pred_3d_vertices_intermediate, gt_3d_vertices_intermediate, has_smpl, args.device))
...
def parse_args():
    ...
    parser.add_argument("--resume_checkpoint", default="/root/fengzehui/output_human36_step8_ema_back_loss/checkpoint-0-0/state_dict.bin", type=str, required=False,
...
def main(args):
    ...
    smpl_intermediate_faces = torch.from_numpy(np.load(path+'src/modeling/data/smpl_1723_faces.npy', encoding='latin1', allow_pickle=True).astype(np.int64)).to(args.device)
    smpl_intermediate_edges = torch.from_numpy(np.load(path+'src/modeling/data/smpl_1723_edges.npy', encoding='latin1', allow_pickle=True).astype(np.int64)).to(args.device)
    ...
    #for name,para in _FastMETRO_Network.named_parameters():
        # with torch.no_grad():
    #    para = para + (torch.rand(para.size())-0.5) * 0.15 * torch.std(para).to(device)
