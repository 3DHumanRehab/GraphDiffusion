...
from src.modeling.model.hrnet_cls_net_featmaps_adapt import get_cls_net
...
def run_train(args, train_dataloader, val_dataloader, FastMETRO_model, smpl, mesh_sampler, renderer, smpl_intermediate_faces, smpl_intermediate_edges):    
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer.module, step_size=args.lr_drop,gamma=0.5)
    ...
    for _, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        loss_3d_vertices = (args.vertices_coarse_loss_weight * (vertices_loss(criterion_3d_vertices, pred_3d_vertices_coarse, gt_3d_vertices_coarse, has_smpl, args.device) +vertices_loss_log(log_3d_vertices, pred_3d_vertices_coarse, gt_3d_vertices_coarse, has_smpl, args.device) +loss_3d_vertices_coarse_mean_velocity)+ \
                        args.vertices_intermediate_loss_weight * (vertices_loss(criterion_3d_vertices, pred_3d_vertices_intermediate, gt_3d_vertices_intermediate, has_smpl, args.device)+ vertices_loss_log(log_3d_vertices, pred_3d_vertices_intermediate, gt_3d_vertices_intermediate, has_smpl, args.device)+loss_3d_vertices_intermediate_mean_velocity)+ \
                        args.vertices_fine_loss_weight * (vertices_loss(criterion_3d_vertices, pred_3d_vertices_fine, gt_3d_vertices_fine, has_smpl, args.device) + vertices_loss_log(log_3d_vertices, pred_3d_vertices_fine, gt_3d_vertices_fine, has_smpl, args.device) +loss_3d_vertices_fine_mean_velocity))
...
def parse_args():
    ...
    parser.add_argument("--resume_checkpoint", default="/root/fengzehui/output_3dpw_step8_ema_back_S_adapt_loss_test/checkpoint-25-4400/state_dict.bin", type=str, required=False,
